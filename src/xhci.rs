...
// and supports up to 256 USB devices or hubs
// [xhci_1_2] p.59
...
impl DeviceContext {
    ...
    fn set_last_valid_dci(&mut self, dci: usize) -> Result<()> {
        ...
        // requires the Context Entries filed to be initialized to
...
struct DeviceContextBaseAddressArray {
    ...
    // NB: the index of context is [slot - 1], no slot.
...
impl CommandRing {
    ...
    pub fn push(&mut self, mut src: GenericTrbEntry) -> Result<u64> {
        ...
        // The returned ptr will be used for wating on command completion
        // events
        Ok(dst_ptr as u64)
    }
}
...
// DO NOT implement Copy trait - this shuould be the only instance to have the
// ptr.
pub struct Doorbell {
    ptr: Mutex<*mut u32>,
}
...
impl SetupStageTrb {
    ...
    pub fn new(request_type: u8, request: u8, value: u16, index: u16, length: u16) -> Self {
        // Table 4-7: USB SETUP Data to Data Strage TRB and Status Stage TRB
        // mapping
...
pub struct EndpointDescriptor {
    ...
    // attributes:
    //   - bit[0..=1]: transfer type(0: out, 1: in)
    //     Interrupt)
    pub attributes: u8,
