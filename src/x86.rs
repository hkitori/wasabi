...
/// # Safety
/// Anything can happen if CS given is invalid.
...
struct InterruptInfo {
    ...
    // Should be aligned on 16-byte boundaries to pas the
...
inthandler_common:
    // General purpose registers (except rsp and rcx)o
    push r15
    push r14
    push r13
    push r12
    ...
    sub rsp, 512 + 8
    fxsave64[rsp]
    // 1st paramter: pointer to the saved CPU state
    mov rdi, rsp
...
extern "sysv64" fn int_handler_unimplemented() {
    panic!("unxpected interrupt!");
}

// PDRDRTTTT (TTTT: type, R: reserved, D: DPL, P: present)
...
#[derive(Copy, Clone)]
enum IdtAttr {
    // Without _NotPresent value, MaybeUnit::zeroed() on
...
impl TaskStateSegment64 {
    ...
    unsafe fn alloc_interrupt_stack() -> u64 {
        ...
        // now, no one expect us own the region since it is forgotten by the allocator ;)
        rsp
    }
    pub fn new() -> Self {
...
/// instane of the reference exist at every momoent.
pub unsafe fn take_current_page_table() -> ManuallyDrop<Box<PML4>> {
    ManuallyDrop::new(Box::from_raw(read_cr3()))
}
/// # Safety
///  This function sets the CR3 value so that anything bad can hppen.
